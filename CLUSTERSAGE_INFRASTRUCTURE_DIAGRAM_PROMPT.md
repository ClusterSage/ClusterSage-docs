...
Canonical domain: nexaflow.site
Root domain: nexaflow.site is the primary production hostname; www.nexaflow.site is an optional alias
```
...
```text
nexaflow.site is served directly; www.nexaflow.site may alias to the same Front Door route
```
...
- `nexaflow.site` as the primary production hostname, with `www.nexaflow.site` as an optional alias on the same Front Door route
...
-> outbound HTTPS
-> https://nexaflow.site/api/agent/register
-> Azure Front Door/WAF
-> kgateway
