...
locals {
  gateway_api_crd_documents = var.deploy_kubernetes ? {
    for manifest in [
  ...
  platform_frontend_image   = coalesce(var.frontend_image_repository, "${module.app_hosting.acr_login_server}/clusterwatch-frontend")
  platform_backend_image    = coalesce(var.backend_image_repository, "${module.app_hosting.acr_login_server}/clusterwatch-backend")
...
resource "helm_release" "clustersage_platform" {
  ...
  create_namespace = true
  chart            = "${path.module}/../deploy/helm/clusterwatch-platform"
  ...
  set_sensitive {
    ...
    value = random_password.agent_token_secret.result
  }
  ...
  depends_on = [
    ...
    azurerm_federated_identity_credential.clusterwatch_workloads,
